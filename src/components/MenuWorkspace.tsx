...
function MenuWorkspaceInner({
  ...
  const uploadedImageIds = menuImages
    ...
    .map((image) => image._id);
  const hasProcessingImages = menuImages.some(
    (image) => image.status === "processing",
  );
  const showProcessingBanner = isProcessing || hasProcessingImages;

  return (
    <div className="flex w-full flex-col gap-6">
      {showProcessingBanner ? <AiProcessingBanner /> : null}
          ...
          <div className="bg-card mt-4 rounded-lg border p-4">
            <div className="mb-3 flex items-center justify-between gap-2">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  {menuImages.length} image
                  {menuImages.length === 1 ? "" : "s"} available
                </p>
              </div>
              <ProcessImagesButton
                restaurantId={restaurantId}
                imageIds={uploadedImageIds}
                disabled={uploadedImageIds.length === 0 || isProcessing}
